...
contract escrowZKMLVerifier is Ownable {
    ...
    Groth16Verifier public myVerifier;
    ...
    constructor(address verifierAddress) Ownable(msg.sender) {
        myVerifier = Groth16Verifier(verifierAddress);
    }
    ...
    function verifyProof(address _client, bytes32 _proof_hash, uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[11] calldata _pubSignals) external onlyOwner returns (bool) {
        require(getProofHash(_pA, _pB, _pC, _pubSignals) == _proof_hash);
        require(client_proofs[_client].proofExistence[_proof_hash]);
        require(client_proofs[_client].proofToEval[_proof_hash]);
        require(client_proofs[_client].proofsRequired > 0);

        (bool success, bytes memory data) = address(myVerifier).staticcall(abi.encodeWithSelector(myVerifier.verifyProof.selector, _pA, _pB, _pC, _pubSignals));
        require(success);
    ...
    function getProofHash(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[11] calldata _pubSignals) public pure returns (bytes32) {
        return sha256(abi.encode(_pA, _pB, _pC, _pubSignals));
    }
