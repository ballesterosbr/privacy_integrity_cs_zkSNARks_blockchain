...
contract escrowZKMLVerifier is Ownable {
    ...
    function verifyProof(address _client, bytes32 _proof_hash, uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[1] calldata _pubSignals) external onlyOwner returns (bool) {
    ...
    function getProofHash(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[1] calldata _pubSignals) public pure returns (bytes32) {
