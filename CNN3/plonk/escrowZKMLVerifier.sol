...
contract escrowZKMLVerifier is Ownable {
    ...
    PlonkVerifier public myVerifier;
    ...
    constructor(address verifierAddress) Ownable(msg.sender) {
        myVerifier = PlonkVerifier(verifierAddress);
    }
    ...
    function verifyProof(address _client, bytes32 _proof_hash, bytes32[24] calldata proof, uint256[1] calldata pubSignals) external onlyOwner returns (bool) {
    ...
    function getProofHash(bytes32[24] calldata proof, uint256[1] calldata pubSignals) public pure returns (bytes32) {
